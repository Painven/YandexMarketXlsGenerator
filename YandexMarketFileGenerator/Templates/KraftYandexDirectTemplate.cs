...
{
    [Description("Шаблон для KRAFT")]
    public class KraftYandexDirectTemplate : IYandexDirectTemplate
    {
        public string Manufacturer { get; } = "Kraft";
        public string Host { get; } = "https://etk-komplekt.ru";
        public string TranslatedManufacturer { get; } = "Крафт";
        ...
        public Dictionary<string, string> ColumnStaticValues { get; } = new Dictionary<string, string>()
        {
            ...
            ["V"] = "+",
            ["X"] = "Работает везде",
            ["Y"] = "Kraft||Наборы инструмента||Домкраты||Пневматический инструмент",
            ["AA"] = "https://etk-komplekt.ru/kraft||https://etk-komplekt.ru/nabory-instrumentov/nabory-instrumentov-kraft/||https://etk-komplekt.ru/specializirovannyy-instrument/instrument-dlya-avtoservisa/domkraty-kraft/||https://etk-komplekt.ru/pnevmaticheskiy-instrument/pnevmaticheskiy-instrument-kraft/"
        };
        ...
        {
            var data = new YandexMarketSection(this, typeof(KraftYandexMarketSectionLine), productInfo, groupIndex);

            return data.BuildSection(3);
        }
    }

    internal class KraftYandexMarketSectionLine : YandexMarketSectionLineBase
    {
        public KraftYandexMarketSectionLine(YandexMarketSection parentSection) : base(parentSection)
        {
        }
        ...
        {
            return $"kraft кт {parentSection.ProductLine.Model}";
        }

        protected override string GetTitle1()
        {
            //Kraft KT 700300. Набор инструментов
            var title = $"Kraft KT {parentSection.ProductLine.Model}. {parentSection.ProductLine.ProductTypeFull}".Trim();

            if(title.Length >= TITLE1_MAX_LENGTH)
            {
                title = title
                    .Replace("Kraft KT ", "Kraft ")
                    .Replace("мм", string.Empty)
                    .Replace("шт.", "шт")
                    .Replace("пр.", "пр")
                    .Replace($"{Product.Model}. ", $"{Product.Model} ");
        ...
        protected override string GetTitle2()
        {
            ...
            var title = $"Kraft KT {parentSection.ProductLine.Model}. {parentSection.ProductLine.ProductTypeShort}".Trim();
        ...
        protected override string GetTitle3()
        {
            ...
            var title = $"Kraft KT {parentSection.ProductLine.Model} {parentSection.ProductLine.Name}".Trim();
        ...
        {            
            ...
            if (lineNumber == 1)
            {
                keyPhrase = $"kraft кт {parentSection.ProductLine.Model}";
            }
            else if (lineNumber == 2)
            {
                keyPhrase = $"кт {parentSection.ProductLine.Model} -kraft";
            }
            else if (lineNumber == 3)
            {
                keyPhrase = $"{parentSection.ProductLine.ProductTypeShort} kraft кт {parentSection.ProductLine.Model}";
            }
            else
            {
        ...
        {
            return $"Kraft-KT-{parentSection.ProductLine.Model}";
        }
    }
}
