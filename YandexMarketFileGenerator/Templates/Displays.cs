...
{
    [Description("Шаблон для Indication")]
    public class IndicationYandexDirectTemplate : IYandexDirectTemplate
    {
        ...
        {
            var data = new YandexMarketSection(this, typeof(IndicationYandexMarketSectionLine), productInfo, groupIndex);

            return data.BuildSection(linesCount);
        }
    }

    internal class IndicationYandexMarketSectionLine : YandexMarketSectionLineBase
    {
        protected string Brand => Product.CustomField;

        public IndicationYandexMarketSectionLine(YandexMarketSection parentSection) : base(parentSection)
        {
        ...
        {
            var title = $"{Model} {Product.ProductTypeShort} {Brand}";
        ...
        {
            var title = $"{ProductTypeFull} {Brand} {Model} (арт. {Sku}) официальный дилер, доставка по России!";
